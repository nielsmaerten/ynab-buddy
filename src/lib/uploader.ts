...
export function upload(parsedFile: ParsedBankFile, config: Configuration) {
  ...
  const transactions = parsedFile.transactions
    .map((tx) => addYnabProps(tx, accountId, flagColor))
    .sort((a, b) => {
  ...
  for (let i = 1; i < transactions.length; i++) {
   ...
   // if (tx.occurrence <= prev_tx.occurrence ) tx.occurrence = prev_tx.occurrence + 1
  }

  return sendToYnab(transactions, budgetId, token);
}

export function sendToYnab(transactions: any[], budgetId: string, token: string) {
  const payload = {
    transactions,
  };
